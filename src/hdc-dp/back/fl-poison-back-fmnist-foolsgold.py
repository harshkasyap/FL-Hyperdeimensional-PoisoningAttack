...
if backdoor_attack["is"]:
    ...
    backdoor_attack["data"] = backdoor_attack["func"](backdoor_attack["data"],
                                                     backdoor_attack["source_label"],
                                                     backdoor_attack["target_label"],
                                                     backdoor_attack["trojan_func"],
                                                     backdoor_attack["percent"])
...
if backdoor_attack["is"]:
    for client in mal_clients:
        clients_data[clients[client]] = backdoor_attack["func"](clients_data[clients[client]],
                                                             backdoor_attack["source_label"],
                                                             backdoor_attack["target_label"],
                                                             backdoor_attack["trojan_func"],
                                                             backdoor_attack["percent"])
...
for epoch in tqdm(range(fedargs.epochs)):
    ...
    if epoch > 0:
        ...
        # Evaluate Backdoor
        if backdoor_attack["is"]:
            backdoor_test_output = fl.backdoor_test(global_model, backdoor_attack["loader"], device, backdoor_attack["source_label"])
            fedargs.tb.add_scalar("Backdoor Success Rate/", backdoor_test_output["accuracy"], epoch)
            wb.log({"epoch": epoch, "time": time.time(), "attack_success_rate": backdoor_test_output["accuracy"], "misclassification_rate": backdoor_test_output["misclass"]})
            log.jsoninfo(backdoor_test_output, "Backdoor Test Outut after Epoch {} of {}".format(epoch, fedargs.epochs))

        # CSV logging
        ltbl.ar({'epoch': epoch,'acc': global_test_output["accuracy"], 'test_loss': global_test_output["test_loss"], 'asr': backdoor_test_output["accuracy"], 'msr': backdoor_test_output["misclass"]})    
