...
if hdc_dp_attack["is"]:
    for client in mal_clients:
        clients_data[clients[client]] = hdc_dp_attack["func"](clients_data[clients[client]], device,
                                                              hdc_dp_attack["args"],
                                                              label_flip_attack["labels"],
                                                              hdc_dp_attack["percent"])
if hdc_dp_attack["is"]:
    with Pool(len(mal_clients)) as p:
        func = partial(hdc_dp_attack["func"], device, hdc_dp_attack["args"], label_flip_attack["labels"], hdc_dp_attack["percent"])
        malicious_clients_data = p.map(func, [clients_data[clients[client]] for client in mal_clients])
        p.close()
        p.join()

    for client in mal_clients:
        clients_data[clients[client]] = malicious_clients_data[client]     
...
@background
def hdc_dp(data, device, hdc_args, flip_labels, poison_percent = 0.5):
    return hdc_dp_attack["func"](data, device, hdc_args, flip_labels, poison_percent)

if hdc_dp_attack["is"]:
    hdc_dps = [hdc_dp(clients_data[clients[client]], 
                      device,
                      hdc_dp_attack["args"],
                      label_flip_attack["labels"],
                      hdc_dp_attack["percent"]) for client in mal_clients]
    try:
        mal_clients_data = fedargs.loop.run_until_complete(asyncio.gather(*hdc_dps))
    ...
    for client, mal_data in zip(mal_clients, mal_clients_data):
